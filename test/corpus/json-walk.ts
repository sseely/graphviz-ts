...
function spawnCapture(
  ...
  return new Promise((resolve) => {
    ...
    // Accumulate raw BYTES and decode once at close. `stdout += d` decodes each
    // Buffer chunk independently, so a multi-byte UTF-8 char split across a chunk
    // boundary (c3 b6 = ö) becomes two U+FFFD — a random, buffering-dependent
    // corruption (1879). Decode the whole stream as UTF-8 (non-fatal).
