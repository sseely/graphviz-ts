...
import { printfFixed } from '../util/printf-round.js';
 ...
 * half-away-from-zero instead. Delegates to the general-purpose
 * {@link printfFixed} (exact-decimal BigInt rounding), which subsumes the
 * `%.Nf` case for arbitrary `decimals`, not just 2.
 *
 * @see lib/gvc/gvdevice.c:gvprintdouble
 */
export function toFixed2HalfEven(n: number): string {
  return printfFixed(n, 2);
}
