...
//
// decorate=true attachment polylines (mission fix-element-count-bucket, T6).
// C draws a 3-point polyline from the label's bottom-right corner along the
// bottom edge, then to the closest point on the spline, with the DEFAULT line
// style and the label fontcolor (emit.c:emit_attachment 1870-1894; gated at
// emit.c:3013 by mapbool(decorate) && ED_spl). Source embedded verbatim
// (graphs/decorate.gv) — red/green: pre-fix the port emitted zero polylines.

import { describe, it, expect } from 'vitest';
import { renderSvg } from '../index.js';

const DECORATE_GV = `digraph states { 
    compound=true;
    labelloc=t;
    fontname=Arial;
    fontsize=9;
...
describe('edge label decorate attachment polylines', () => {
  const svg = renderSvg(DECORATE_GV, 'dot');
  it('emits one 3-point polyline per decorated labeled edge', () => {
    const polys = svg.match(/<polyline[^>]*>/g) ?? [];
    expect(polys.length).toBe(26);
    for (const p of polys) {
      expect(p).toContain('fill="none" stroke="black"');
      expect((p.match(/-?\d+\.?\d*,-?\d+\.?\d*/g) ?? []).length).toBe(3);
    }
  });
});
