...
| 2026-06-21 | T5+T6 | VERIFIED byte-match vs native oracle for dot/odot/crow/vee/diamond/box; full suite 2233 green incl. golden suite (0 normal regression) | end-to-end confirmation the G1/G2 targets are solved before the T8 corpus survey |
| 2026-06-21 | T8 | parity regen (dot 15.1.0): byte-match 245→249 (+4), structural 219→222 (+3), diverged 295→288 (−7); **0 per-id regressions** (HARD GATE passed) | ADR-6 success metric met |
...
- **Parity:** byte-match 245→**249**, structural 219→**222**, diverged 295→**288**;
  **0 regressions**. 8 corpus improvements; G1 (dot/odot `<ellipse>`) + G2 (crow/vee
  9-pt) symptoms fixed, byte-matched in isolation + via 8 new goldens (manifest 146→154).
