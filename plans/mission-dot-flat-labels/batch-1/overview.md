...
|----|-------------|-------|--------|------------|------|
| T1 | Wire real `flatEdges`; fix `needsAbomination` (flat_out); rewrite `abomination` 0-based; align `flatNode`/`makeVnSlot` indexing | opus | `src/layout/dot/position.ts`, `src/layout/dot/flat.ts` | — | [x] |
