...
| 2026-06-23 | X2 gate | **SURVEY GATE PASS**: conformant 312→312, structural 256→256, **0/796 changed verdicts** vs parity-baseline. typecheck 0, full test 2334 pass (+5), build 0. | AD-3 byte-identity holds corpus-wide. |
...
| 2026-06-23 | D3 | **VERDICT: per-op constant factor (AD-2 Path 2), NOT iteration-count gap.** Every structural counter conformant port==C: 2108 reorderInnerIters=1,591,556,868 (both), reorderExchanges=484,932,627 (both), transposeRounds/Steps/Swaps + ncross/rcross/reorder calls all exact; per-pass `mincross:` Verbose trace conformant. Only diff: port accumCrossPairs = EXACTLY ½ C (single-pass both-direction accumCross — port already more efficient). Target=reorderInner + reorderFindLp/Rp/left2right per-op cost (1.6B faithful iters, V8 property-access bound). | Iteration-count fix path disproven; algorithm change forbidden. ≤3× target was contingent on iteration gap → expect only a constant-factor drop. findings.md has full side-by-side. Proceeding to Batch 2 (byte-safe per-op opt, auto-proceed per human). |
