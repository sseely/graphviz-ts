...
SHARED non-adjacent flat path; cnt=1 is conformant by construction (AD-1), but
...
3. **End-to-end confirm:** `render-one.ts` synthetic cnt=2/cnt=3/bottom conformant
...
5. **Close:** update memory `flat-edge-241-is-y-only` latent-follow-up note to mark
   the cnt≥2 gap CLOSED (cite the synthetic conformant + zero corpus impact); write the
...
- `plans/nonadjacent-flat-multi/findings-regression.md` (Create) — per-id delta,
  synthetic conformant before/after, oracle restore note.
...
## Acceptance criteria
- `vitest` 0 failures; goldens conformant out-of-family; synthetic cnt≥2 tests pass.
- `survey.ts`: ZERO new diverges/regressions; 74 cnt=1 flats unchanged; per-id delta
  recorded.
- Synthetic cnt=2/cnt=3/bottom conformant with native end-to-end.
