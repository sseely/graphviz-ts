...
pinned in `edge-route-splines.test.ts`. 115 goldens conformant.
...
- T3: parallel/opposing groups (`routeParallelEdgeGroup`) route the shared base
  ...
  reversed to tail→head. Opposing `b->a` is conformant to the dot oracle.
...
edge `<path>` data **conformant to `dot` 15.0.0**, including the `a→c`
...
the ported pathplan and match dot 15.0.0 conformant. Remaining flat sub-gaps
...
label is emitted at the dot 15.0.0 position (72, −32.91) conformant. The
...
But the emitted position is NOT conformant: it inherits an upstream divergence
in how the rotated aux pipeline lays out a *labeled* cross-rank edge — see
DOT-11. A no-port-no-label ported flat is conformant to dot 15.0.0
...
conformant. T2 code for the copy-back was implemented and reverted pending
...
edge conformant before the copy-back can land the label correctly).
...
labeled-flat spline conformant to dot 15.0.0 and corrected the label X. The
...
ported flat is conformant, so the divergence is label-specific and lives in the
...
**Downstream visual impact:** LOW — narrow case, but blocks DOT-10's conformant
...
instrumentation: `map_point`/`place_vnlabel`/dispatch are all conformant
...
the aux labels in `makeFlatAdjEdges`. Label now conformant at (72, −32.91).
...
is conformant. Fully traced — NOT a label-sizing or placement bug:
- `placeVnlabel` (`splines-label.ts`) is conformant to C `place_vnlabel`
...
**Downstream visual impact:** LOW — narrow case; blocks DOT-10's conformant
label.
