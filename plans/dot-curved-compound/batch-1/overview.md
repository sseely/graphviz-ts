...
|----|-------------|-------|--------|------------|------|
| T1 | Port `makeStraightEdges`(+`bend`,`get_cycle_centroid`) → `straight-edges.ts`; dispatch `EDGETYPE_CURVED`; resetRW/warn/finish wiring | sonnet | `src/layout/dot/straight-edges.ts`, `src/layout/dot/splines.ts` (and/or `edge-route-chain.ts`), `src/layout/dot/curved.test.ts` | — | [ ] |
