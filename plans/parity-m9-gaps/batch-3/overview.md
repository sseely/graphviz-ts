...
¹ Creation + AD6 typing + angle-guarded place_portlabel landed and
committed (2 agent deaths; orchestrator finished inline). Default-attr
placement requires the UNPORTED lib/label xlabels subsystem (~1756
lines) → STOP condition; dot-head-tail-label stays quarantined.
² Promotion blocked by the T9 stop; close bookkeeping executed in
reduced form (this file, README summary, journal). Manifest stays 66.
