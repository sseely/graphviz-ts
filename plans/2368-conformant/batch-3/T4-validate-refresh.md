...
Final batch. Confirm 2368 conformant (or the documented ≤1pt residual per AD-3)
...
2. Assert GATE PASS, 0 regressions, 0 clip-regressions; assert 2368 is
   `conformant` (goal) OR `diverged` with only the documented ≤1pt x-NS residual.
...
- Given the parity report, then 2368 is `conformant` (mission goal) OR `diverged`
  with ONLY the documented ≤1pt x-NS residual (AD-3 outcome).
- Given the refresh, when `dashboard.ts` runs, then `parity.json`,
  `parity-rules.json`, `PARITY.md` reflect the new state and the conformant count
...
`chore: refresh parity baseline after 2368 conformant`.
