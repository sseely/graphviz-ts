...
sits at C's Y. The 256→316 label vnode is conformant to C, so the fix is in
...
Batch 1 (drawing those arcs) grew the bbox; every 2368 node Y now conforms to C
...
differs by ~5pt), re-render 2368, run the full survey gate. Any conformant→worse
is STOP + revert (AD-4). Batch done = 2368 bbox = C (608×148) AND survey 0
regressions AND 2368_1 + 1624 still conformant. NOTE: the write-set is one of two
