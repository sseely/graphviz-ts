...
|------|------|------------------------|-----------|
| start | — | Branch `feature/2368-conformant` created off `main`. Pre-existing working-tree changes (settings + untracked notes) left untouched; task commits scoped to declared write-sets only. | Protocol startup. |
...
| B1 | T1 | 2368 residual after B1: height conformant; width Δ4 + arc Δ (376→76=11.22) all downstream of pre-existing x-NS node deltas (Issue 3): equal-shift pairs (256→436, both Δ4) match C modulo shift; differential-shift {76,376} (76 Δ0/376 Δ2) distorts the box → spline amplifies. Added opposing-pair unit test. tsc+2468 vitest pass. | 2368 conformant blocked only by Issue 3 (Batch 3, AD-3 conditional). |
...
| B2 | T2 | **NO-OP** (per AD-2 / T0 hypothesis, now confirmed). Issue 1 (vspace) was a consequence of Issue 2, fully resolved by Batch 1: every 2368 node Y conforms to C (top group line7/136 at -140.35; height 148; ty 144.35). No `flat.ts`/`position-ycoords.ts` change needed. No code change → no survey. | Batch-2 vspace target (608×148 height, top group at C's Y) achieved by B1; remaining width Δ4 is Issue 3. |
...
| B3 | T3 | Fix: rewrote flat.ts flat_edges dist pass faithful to C — `processFlatOutLabel` (dist on rep) + `processOtherLabel` (resolve rep via to_virt, copy adjacency, MAX dist onto rep, rank/self guards), both gated on flat_out existing. RESULT: 2368 bbox conformant (608×148) + EVERY node x conformant C; 256→436/376→196 down-arcs now conformant. GATE PASS: stable=690, 0 regressions. | Huge improvement; far beyond AD-3's "≤1pt residual" target. |
...
conformant C. Survey: stable 689→691, 0 regressions across all batches.
