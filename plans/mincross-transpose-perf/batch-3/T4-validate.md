...
byte-identical to C (AD-1), and leaves a permanent regression so the win cannot
silently rot.

## Task

1. Render `tests/2471.dot` through TS to completion; record wall-clock.
2. Verify per-rank order **byte-identical to C** on mc3, the 6-cluster chain,
...
- **Given** mc3 + 6-cluster chain + mid-size benchmark, **when** compared to the
  C oracle, **then** per-rank order is **byte-identical**.
