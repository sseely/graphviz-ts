...
|---|---|---|---|---|---|
| T3 | Faithful fix at the mechanism's origin + regression test | main session (opus) | PROVISIONAL: `src/layout/dot/{edge-route*.ts, splines-route*.ts}`, `src/common/splines-clip.ts` + matching `.test.ts` — subject to the write-set expansion protocol (decisions.md) driven by T2 `fixLocus` | T2 | [ ] |
| T4 | NaN-family + watch-graph gate | main session | `.agent-notes/` only | T3 | [ ] |
