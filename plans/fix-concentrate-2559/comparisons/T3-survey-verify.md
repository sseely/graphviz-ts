...
|----|------------------------|------------------------------|--------|
| **2559** | `diverged` (maxΔ=0) | **`conformant`** | ✅ diverged → conformant (exceeds the structural-match bar, ADR-3) |
...
Port output is conformant to the headless 15.1.0 SVG content (differs only in
...
|------------|-------|------|
| structural-match → conformant | 63 | improvement (more exact) |
| diverged → structural-match | 11 | improvement |
| diverged → conformant | 2 | improvement (incl. 2559) |
| conformant → structural-match | 7 | sub-pixel downgrade (≤1px, still matching) |
| structural-match → diverged | 3 | **allowlisted, not caused by this change** |

Net: **conformant +56, total match +10, diverged −10**, zero hard regressions.
...
ADR-3 bar). Net conformant still rose by 56.
...
|----|----------|----------|------|
| 167 | conformant | conformant | unchanged |
| 2087 | conformant | conformant | unchanged |
...
| b135 | conformant | conformant | unchanged (port 1 = headless 1) |
| b62 | conformant | conformant | unchanged (port 1 = headless 1) |
| b71 | conformant | conformant | unchanged (port 71 = headless 71) |
