...
them conformant — measure first (Batch 1), then migrate category-by-category,
...
tested (flat labels matched dot conformant via the same pathplan).

## AD-3: Goldens conformant is the invariant; new cases pinned as oracles

**Context:** dot-edge-multi / dot-flat-labels established the oracle-pin pattern.
**Decision:** After each migration, the 115 goldens must stay conformant; the
