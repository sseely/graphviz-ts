...
| 2026-06-20 | T2 | `2222.dot` conformant→timeout in one survey run is flaky, NOT a regression | 28k-line, point-free graph renders in ~13.7s alone but exceeds the 20s wall-clock under concurrency-8; confirmed `compareSvg` pass:true in isolation. Re-run survey returned it to conformant (timeout 6, baseline). |
