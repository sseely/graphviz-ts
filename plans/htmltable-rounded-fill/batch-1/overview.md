...
| T2 | Oracle-pin 5 grd* → byte-match, add golden, regenerate survey | general-purpose | `test/golden/dot-htmltable-rounded-grad.*`, `test/golden/manifest.json`, `test/golden/suite.test.ts`, `test/corpus/parity.json`, `test/corpus/PARITY.md`, `plans/htmltable-rounded-fill/decision-journal.md` | T1 | [x] |
...
3. T2 — render the 5 grd* corpus cases, byte-diff against the native oracle,
   confirm they reach byte-match, add a golden, regenerate the parity survey.
